...
_date_fm2_pat = _whole_word(r'\d{2} (Jan|Feb|Mar|Apr|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')

_test_date_fm2_pat = re.compile(r'\d{2} (Jan|Feb|Mar|Apr|Jul|Aug|Sep|Oct|Nov|Dec)(,?) \d{4}')
_test_date_iso8601_pat = re.compile(r'\d{4}-(0\d|1[0-2])-(0[1-9]|[12][0-9]|3[01])( |T)\d{2}:\d{2}:\d{2}.\d{3}(([A-Z]{3}|[A-Z])+|-\d{3})*')
_test_integer_pat = re.compile(r'(\d{1,3},?)+($| )')
...
def dates_iso8601(text):
    '''Find date in pattern like 2015-13-48'''
    for match in _date_iso8601_pat.finditer(text):
        yield('ordinal', match)

def test_dates_iso8601(text):
    '''Find date in pattern like 2015-13-48'''
    for match in _test_date_iso8601_pat.finditer(text):
        yield('ordinal', match)

def dates_fmt2(text):
    '''Find date in pattern like 25 Jan 2018'''
    for match in _date_fm2_pat.finditer(text):
        yield('ordinal', match)
...
def test_integers(text):
    '''Find integers in text. Don't count floating point numbers.'''
    for match in _test_integer_pat.finditer(text):
        # If the integer we're looking at is part of a floating-point number, skip it.
...
def test_dates_fmt2(text):
    '''Find date in pattern like 25 Jan, 2018'''
    for match in _test_date_fm2_pat.finditer(text):
        yield('ordinal', match)
